...
"""
# Author: zhao zhishan
# Created Time : 2021-01-16 06:50
# File Name: models/nfm.py
# Description:
"""
...
class Model(BaseModel):
    ...
    def _model_fn(self, feat_idx, feat_val):
        ...
        with tf.variable_scope('nfm', reuse=tf.AUTO_REUSE):
            ...
            second_order_output = 0.5 * (tf.subtract(square_sum, sum_square))
            second_order_output = dropout_layer(second_order_output,
                                                self._params['dropout_rate'],
                                                self.is_train)
            # dnn
            deep_logits = deep_layer(second_order_output,
                                     ...
                                     output_bias=False)
            # sum
            logits = tf.add_n([first_order_output, deep_logits]) + bias
